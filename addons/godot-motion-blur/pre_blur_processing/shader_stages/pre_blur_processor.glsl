...
void main() {
	...
	mat4 read_view_matrix = transpose(
		mat4(
	...
	mat4 read_prev_view_matrix = transpose(
		mat4(
			previous_scene_data.view_matrix[0],
	...
	vec3 base_velocity = vec3(
		textureLod(vector_sampler, uvn, 0.0).xy + mix(vec2(0), camera_uv_change.xy, step(depth, 0.)),
	...
	* sharp_step(
	...
	total_velocity *= clamp_size / max(clamp_size, total_velocity_length);

	vec4 final_output = vec4(total_velocity, depth == 0 ? (-1.0 / 0.0) : view_position.z);
	...
	imageStore(debug_4_image, uvi, final_output);
#endif
